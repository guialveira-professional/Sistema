...
class Groups(Base):
    ...
    def post(self,request):
        ...
        if not name:
            raise RequiredFields #Erro de campos necessários
        ...
        if permissions:
            ...
            try:
                for item in permissions:
                    ...
                    if not Group_Permission.objects.filter(group_id=created_group.id, permission_id=item).exists():
                        Group_Permission.objects.create(
        ...
        if permissions:
            ...
            try:
                for item in permissions:
                    ...
                    if not Group_Permission.objects.filter(group_id=group_id, permission_id=item).exists():
                        Group_Permission.objects.create(
                            group_id = group_id,
                            permission_id = item
                        )
