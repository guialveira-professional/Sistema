...
class EmployeesPermissions(permissions.BasePermission):
    message = 'O funcionário não tem permissão de gerenciamento de funcionários'
...
class GroupsPermission(permissions.BasePermission):
    message = 'O funcionário não tem permissão de gerenciamento de grupos'
...
class GroupsPermissionsPermission(permissions.BasePermission):
    message = 'O funcionário não tem permissão de gerenciamento de permissões'

    def has_permission(self, request, _view):
        return check_permission(request.user, request.method, permission_to='permission')
    
class TaskPermission(permissions.BasePermission):
    message = 'O funcionário não tem permissão de gerenciamento de tarefas globais'
